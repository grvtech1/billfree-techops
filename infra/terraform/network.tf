...
# One security group for the whole cluster: full mesh between nodes (kubeadm +
# Calico need many inter-node ports), plus the externally reachable ports.
resource "aws_security_group" "cluster" {
  ...
  ingress {
    description = "Ingress HTTP/HTTPS (ingress-nginx via NodePort/hostPort)"
    from_port   = 80
    to_port     = 443
    protocol    = "tcp"
